    ...
    >
      ...
      >
        <button ref={closeButtonRef} type="button"
          className="absolute right-4 top-4 z-10 grid size-11 place-items-center rounded-full bg-black/70 transition hover:bg-acid hover:text-ink"
          aria-label="Close movie details" onClick={onClose}>
          <CloseIcon />
        </button>
        <div
          className={`min-h-72 bg-cover bg-center p-8 md:min-h-[36rem] ${getPosterClass(movie.art)}`}
        ...
        <div className="p-7 md:p-12">
          ...
          <dl className="mt-8 grid gap-5 border-t border-white/10 pt-7 sm:grid-cols-2">
            ...
            <div><dt className="text-xs uppercase tracking-widest text-muted">Rating</dt><dd className="mt-2 text-acid">{movie.rating ? `★ ${movie.rating} / 10` : "Not rated"}</dd></div>
          </dl>
          <button
            className={`mt-8 flex w-full items-center justify-center gap-3 rounded-sm px-5 py-4 font-bold transition ${
              isFavorite ? "bg-acid text-ink" : "border border-white/20 hover:border-acid hover:text-acid"
            }`}
            type="button"
